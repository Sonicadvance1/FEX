...
bool HasIOCTL32();
  ...
  void RegisterSocket() {
#if _M_ARM_64
    constexpr uint32_t arm_SYS_sendmsg = 296;
    constexpr uint32_t arm_SYS_recvmsg = 297;
    if (HasIOCTL32()) {
      REGISTER_SYSCALL_IMPL_X32(socketcall, [](FEXCore::Core::CpuStateFrame *Frame, uint32_t call, uint32_t *Arguments) -> uint64_t {
        ...
        switch (call) {
          ...
          case OP_SENDMSG: {
            return syscall32(arm_SYS_sendmsg, Arguments[0], Arguments[1], Arguments[2]);
            break;
          }
          case OP_RECVMSG: {
            return syscall32(arm_SYS_recvmsg, Arguments[0], Arguments[1], Arguments[2]);
            break;
          }
          default:
