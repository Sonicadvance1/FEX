...
namespace Alloc::OSAllocator {
  ...
  class OSAllocator_64Bit final : public Alloc::HostAllocator {
    public:
      OSAllocator_64Bit();
      OSAllocator_64Bit(std::vector<FEXCore::Allocator::MemoryRegion> *MemoryRegions);
  ...
  auto CheckIfRangeFits = [&AllocatedOffset](LiveVMARegion *Region, uint64_t length, int prot, int flags, int fd, off_t offset, uint64_t StartingPosition = 0) -> std::pair<LiveVMARegion*, void*> {
    ...
    if (Region->FreeSpace >= length) {
      ...
      if (Region->HadMunmap) {
        ...
        auto SearchResult = Region->UsedPages.BackwardScanForRange<true>(LastAllocation,NumberOfPages, RegionNumberOfPages);
...
OSAllocator_64Bit::OSAllocator_64Bit(std::vector<FEXCore::Allocator::MemoryRegion> *MemoryRegions) {
  AllocateMemoryRegions(*MemoryRegions);
  MemoryRegions->clear();
}
...
std::unique_ptr<Alloc::HostAllocator> Create64BitAllocatorWithMemoryRegions(std::vector<FEXCore::Allocator::MemoryRegion> *MemoryRegions) {
  return std::make_unique<OSAllocator_64Bit>(MemoryRegions);
}
